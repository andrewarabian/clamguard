...
static void config_defaults(Config *c) {
    ...
    c->accurate=0; c->no_update=0; c->i_accept_risk=0; c->noninteractive=0;
}
...
static void process_detections(const Config *cfg,
    ...
    for (int i=0;i<g_found_n;i++) {
        ...
        } else if (strcmp(cfg->action,"remove")==0) {
            if (unlink(path)==0) {
                printf("  %s%s[DEL ]%s %s\n",BOLD,HI,RST,path);
                if (report) fprintf(report,"  [DEL ] %s  sig=%s\n",path,sig);
            } else {
                printf("  %s%s[ERR ]%s delete failed: %s: %s\n",BOLD,HI,RST,path,strerror(errno));
                if (report) fprintf(report,"  [ERR ] delete failed: %s\n",path);
            }
        }
    }
...
static void print_summary(const Config *cfg, long scanned, int found_n,
    ...
    hline(cols);
    if (found_n==0)
        printf("  %s%sRESULT: Clean — no detections%s\n",BOLD,OK,RST);
    else
        printf("  %s%sRESULT: %d DETECTION(S) FOUND%s\n",BOLD,HI,found_n,RST);
    hline(cols);
    printf("\n");
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i=1;i<argc;i++) {
        ...
        else if (!strcmp(argv[i],"--accurate"))                       { cfg.accurate=1;      cfg.noninteractive=1; }
        else if (!strcmp(argv[i],"--no-update"))                      { cfg.no_update=1;     cfg.noninteractive=1; }
        else if (!strcmp(argv[i],"--i-accept-risk"))                  { cfg.i_accept_risk=1; cfg.noninteractive=1; }
        ...
        else if (!strcmp(argv[i],"--exclude") &&i+1<argc) { snprintf(cfg.excludes,sizeof(cfg.excludes),"%s",argv[++i]); cfg.noninteractive=1; }
        else if (!strcmp(argv[i],"--action")  &&i+1<argc) { snprintf(cfg.action,  sizeof(cfg.action),  "%s",argv[++i]); cfg.noninteractive=1; }
    }

    /* apply profile defaults when excludes not explicitly set */
    if (!cfg.noninteractive||cfg.excludes[0]=='\0') {
